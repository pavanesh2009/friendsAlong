...
$sql_insert = "INSERT into `messages`
		    ...
		    VALUES
				('$data[post_title]',
				'$data[post_body]',
				'$user_id','$recipient_id', now(),'$flag','$is_deleted')";
...
<?	
	...
	<div class="trip_form" style="width:750px; border:1px dotted #ccc; margin:0 auto;">
		<?php 
			$sqlquery = mysql_query("select u.full_name, m.message_id, m.title, m.body, m.thread_id from messages m join users u 
				on m.to_id = u.user_id and m.from_id = '$user_id' and m.to_id = '$recipient_id' and m.message_id = '$msg_id'");
				while($msgchain = mysql_fetch_array($sqlquery))
					...
					if(count($msgchain) > 0)
					{
						...
						print "<b>To:</b><p class=\"msgtext\">".$msgchain['full_name']."</p>";
	...
	<tr>
	<td>Subject:</td>
	<td><input type="text" name="post_title" autofocus="on" value="<?php echo isset($_POST['post_title']) ? $_POST['post_title'] : ''; ?>"/></td>
	</tr>
	<tr>
	<td>Message:</td>
	<!--<div id="showData"></div>-->
